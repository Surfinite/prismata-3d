...
4. **Bot deployment**: Runs on `prismata-data` (t4g.micro, ARM/Graviton, on-demand, Linux). Deployed via SSH alongside existing `<SPECTATOR_SERVICE>` spectator service. Python process managed by systemd.
